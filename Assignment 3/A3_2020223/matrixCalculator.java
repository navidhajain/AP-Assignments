...
public class matrixCalculator {
    ...
    void addition(Matrix matt1, Matrix matt2){
        ...
        double[][] mat3=new double[matt1.getnumrows()][matt1.columns];

        if (matt1.columns!=matt2.columns || matt1.rows!=matt2.rows) {
            System.out.println("matrices have different dimensions");
            return;
        }

        for (int i = 0; i < matt1.rows; i++) {
            for (int j = 0; j < matt1.columns; j++) {
    ...
    void subtraction(Matrix matt1, Matrix matt2){
        double[][] mat1=matt1.getmatrix();
        double[][] mat2=matt2.getmatrix();
        double[][] mat3=new double[matt1.rows][matt1.columns];
    ...
    void multiplication(Matrix matt1, Matrix matt2){
        double[][] mat1=matt1.getmatrix();
        double[][] mat2=matt2.getmatrix();
        double[][] mat3=new double[matt1.rows][matt2.columns];
        ...
        for (int i = 0; i < matt1.rows; i++) {
            for (int j = 0; j < matt2.columns; j++) {
                mat3[i][j]=0;
                for (int k = 0; k < matt2.rows; k++)
    ...
    void division(Matrix matt1, Matrix matt2){
        int[][] mat3=new int[matt1.rows][matt2.columns];
    ...
    void solvelineareqns(Matrix matt1, Matrix matt2){    
        ...
        if (!matt1.isitNull() && matt2.isitNull()) {
            int[][] nullmat=new int[matt2.rows][1];
            printmatrix(nullmat);
            return;
        }
        double[][] invmat1=matt1.getinverse();
        Matrix inversemat1=new Matrix();
        inversemat1.setvalues(matt1.rows, matt1.columns, invmat1);
    ...
    void elementmult(Matrix matt1, Matrix matt2){
        ...
        if (matt1.rows!=matt2.rows || matt1.columns!=matt2.columns) {
